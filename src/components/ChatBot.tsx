                                    ...
                                    >
                                      <div className="space-y-1.5 text-[15px] leading-8 text-[#606060]">
                                        {card.split('\n').map((line, lineIdx) => {
                                          ...
                                          // Check if line follows the pattern "**Label:** Value"
                                          const labelMatch = trimmedLine.match(/^\*\*([^:]+):\*\*\s*(.+)$/) || trimmedLine.match(/^\*\*([^:]+):\s*(.+)$/);
                                          if (labelMatch) {
                                            const [, label, value] = labelMatch;
                                            return (
                                              <div key={lineIdx} className="flex">
                                                <span className="font-semibold text-gray-700 min-w-[80px]">{label}:</span>
                                                <span className="font-bold text-gray-900">{value}</span>
                                              </div>
                                            );
                                          }
                                          
                                          // For other lines, render normally with markdown
                                          return (
                                            <div 
                                              key={lineIdx}
              ...
              {isThinking && (
                <div className="flex justify-start">
                  <div className="w-full">
                    ...
                    <div className="inline-flex items-center gap-2 rounded-full bg-card/80 border border-border px-3 py-2">
